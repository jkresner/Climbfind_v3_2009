...
var gVAM_DTTBIds = null;function VAM_InitSpinner(pObj) 
{var vTBFld = VAM_GetById(pObj.CID);if (!vTBFld || vTBFld.Spin)return;vTBFld.Spin = pObj;var vBtnFld = VAM_GetById(pObj.CID + "_Btns");if (vBtnFld != null){
if (gVAM_DTTBIds == null){gVAM_DTTBIds = new Array();}VAM_MoveSpinner(pObj.CID); gVAM_DTTBIds[gVAM_DTTBIds.length] = pObj.CID; }} 
...
function VAM_BtnMouseDown(pBtnId, pGrp){var vBtn = VAM_GetById(pBtnId);if ((vBtn.disabled == null) || !vBtn.disabled){var vX = VAM_GetOffsetX(vBtn, vBtn.offsetLeft, 1);var vY = VAM_GetOffsetY(vBtn, vBtn.offsetTop, 1);if (VAM_ValidateGroup(pGrp, true)){if ((vX != VAM_GetOffsetX(vBtn, vBtn.offsetLeft, 1)) || (vY != VAM_GetOffsetY(vBtn, vBtn.offsetTop, 1))) 
{window.setTimeout("VAM_GetById('" + pBtnId + "').click();", 250);}}}} 
