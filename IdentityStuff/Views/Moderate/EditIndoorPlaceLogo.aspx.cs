using System;
using ClimbFind.Model.Objects;
using ClimbFind.Web.Mvc.Controllers;
using ClimbFind.Web.UI;

namespace IdentityStuff.Views.Moderate
{
    public partial class EditIndoorPlaceLogo : ClimbFindViewPage<IndoorPlace>
    {
        public IndoorPlace place { get { return ViewData.Model; } }

        protected void Page_Load(Object sender, EventArgs e)
        {
            
        }
      
        public void UploadImage_Click(Object Src, EventArgs E)
        {
            if (ImageUploadUC.HasFile)
            {
                //-- save the image:
                cfController.SaveIndoorPlaceLogo(place, ImageUploadUC.FileName, ImageUploadUC.FileBytes);

                RedirectTo<PlacesController>(c => c.DetailIndoor(place.FriendlyUrlLocation, place.FriendlyUrlName));
            }
        }
    }
}
