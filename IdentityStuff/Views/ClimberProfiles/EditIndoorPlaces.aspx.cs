...
{
    public partial class EditIndoorPlaces : ClimbFindViewPage<ClimberProfileViewData>
    {        
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            List<Place> homeIndoorPlaces = (from c in allPlaces where c.CountryID == countryID && c.IsIndoor orderby c.CountryID descending, c.Name select c).ToList();
            List<Place> otherIndoorPlaces = (from c in allPlaces where c.CountryID != countryID && c.IsIndoor orderby c.CountryID descending, c.Name select c).ToList();
            List<Place> indoorPlaces = homeIndoorPlaces;
            indoorPlaces.AddRange(otherIndoorPlaces);
        ...
        protected void UpdateClimberProfile_Click(object sender, EventArgs e)
        {
            ...
            {
                List<int> outdoorPlaces = (from c in climberProfile.PlacesUserClimbs where !c.IsIndoor select c.ID).ToList();

                List<int> allNewPlaceIDs = GetSelectedPlacesUserClimbsAt();
                allNewPlaceIDs.AddRange(outdoorPlaces);
