...
{var vFd = false; var vL = VAM_GetById(vIC.LID); if (vL && (vL.selectedIndex != null) && 
(vIC.CFSI != null)) 
{for (vJ = 0; !vFd && (vJ < vIC.CFSI.length); vJ++){var vCFSI = vIC.CFSI[vJ];var vSI = vL.selectedIndex; if ((vCFSI.SI <= vSI) && (vCFSI.EI >= vSI)){vFd = true;if (vCFSI.Const == null) 
vVal = NaN;elsevVal = vCFSI.Const;}} 
}if (!vFd)if (vIC.ConstNM == null) 
vVal = NaN;elsevVal = vIC.ConstNM;}
else if (vIC.CID != null) 
{
var vTB = VAM_GetById(vIC.CID);if (!vTB || (vTB.value == null)) 
vVal = vIC.BlkZ ? 0.0 : NaN;else{
vVal = VAM_GetDTTBValue(vIC.CID);if (vVal == null) 
{if (VAM_Trim(vTB.value) == "") 
vVal = vIC.BlkZ ? 0.0 : NaN;else 
...
pVal = VAM_Round(pVal, 0, 0);}VAM_SOCSet(vF.id, null); VAM_SetDTTBValue(vF.id, pVal);}}else{vF.className = pAO.InvCss;var vIL = pAO.InvLbl;if (pAO.SVT != 0) 
{if (vIL == "") 
